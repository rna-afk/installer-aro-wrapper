...
func (m *manager) applyIgnitionConfigCustomisations(g graph.Graph) (graph.Graph, error) {
	...
	aroDNSConfig := g.Get(&bootkube.ARODNSConfig{}).(*bootkube.ARODNSConfig)
	...
	err = bootstrap.AddSystemdUnits(bootstrapFile.Config, "bootstrap/systemd/units", temp, newServices)
